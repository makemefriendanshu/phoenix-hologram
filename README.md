...
- `/premiere`, `/premiere/:id` — plays a movie with live "who's in focus" voting (multiple faces per scene, vote count and full per-vote timestamp history shown for each face), comments (replies + likes), a like button, and a quality selector (original vs. low-bitrate preview) for playback and downloads (full movie, or independently-retryable parts via a dropdown). Over the ngrok tunnel, large videos are bandwidth-capped (free tier), so playback can stall on multi-GB files — fine on localhost.
