...
- `/login`, `/register`, `/dashboard`, `/upload` — visual-only account pages (no user/auth system exists yet): styled forms that don't authenticate anyone, and a dashboard/upload page shown with fixed sample data rather than a real account's. Buttons link to whatever's actually real today (Admin View, Pricing/Upgrade) instead of fake flows, with an honest "coming soon" note where nothing exists yet.
